...
def gerar_grafico(df, nivel, titulo):
    ...
    df_contagem = df_contagem.reset_index().melt(id_vars=nivel, var_name='Escolaridade', value_name='Quantidade')
    fig = px.bar(df_contagem, x=nivel, y='Quantidade', color='Escolaridade',
...
def gerar_grafico_formacao(df, nivel, titulo):
    ...
    df_contagem = df_contagem.reset_index().melt(id_vars=nivel, var_name='Formação', value_name='Quantidade')
    fig = px.bar(df_contagem, x=nivel, y='Quantidade', color='Formação',
                 title=titulo, barmode='stack', color_discrete_sequence=cores_personalizadas)
    return fig

def gerar_grafico_carac_org(df, nivel, titulo):
    df_contagem = df.groupby([nivel, 'AssSoc_CaracOrgGest']).size().unstack(fill_value=0)
    df_contagem = df_contagem.reset_index().melt(id_vars=nivel, var_name='AssSoc_CaracOrgGest', value_name='Quantidade')
    fig = px.bar(df_contagem, x=nivel, y='Quantidade', color='AssSoc_CaracOrgGest',
                 title=titulo, barmode='stack', color_discrete_sequence=cores_personalizadas)
    return fig
